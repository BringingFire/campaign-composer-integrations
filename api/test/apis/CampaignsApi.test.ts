import { CampaignsApi, Configuration, DocumentsApi, EntitiesApi, FoldersApi, ImagesApi, LinksApi, MapsApi } from 'campaign-composer-api';
import { TEST_API_KEY, TEST_CAMPAIGNS_EXPECT_LENGTH, TEST_CAMPAIGN_ID, TEST_DOCUMENTS_EXPECT_LENGTH, TEST_DOCUMENT_ID, TEST_ENTITIES_EXPECT_LENGTH, TEST_ENTITIES_WITH_FOLDER_EXPECT_LENGTH, TEST_ENTITY_ID, TEST_FOLDERS_EXPECT_LENGTH, TEST_FOLDER_ID, TEST_HOST, TEST_IMAGES_EXPECT_LENGTH, TEST_IMAGE_ID, TEST_LINKS_EXPECT_LENGTH, TEST_LINK_ID, TEST_MAPS_EXPECT_LENGTH, TEST_MAP_EXPECT_WIDTH, TEST_MAP_ID } from './data';
...
describe('documents', () => {
    ...
    test('get', async () => {
        const res = await api.getDocument({ campaignId: TEST_CAMPAIGN_ID, docId: TEST_DOCUMENT_ID });
...
describe('maps', () => {
    ...
    test('get', async () => {
        const res = await api.getMap({ campaignId: TEST_CAMPAIGN_ID, mapId: TEST_MAP_ID });
        expect(res.id).toBe(TEST_MAP_ID);
    });

    test('get map background', async () => {
        const res = await api.getMapBackground({ campaignId: TEST_CAMPAIGN_ID, mapId: TEST_MAP_ID });
...
describe('folders', () => {
    ...
    test('get', async () => {
        const res = await api.getFolder({ campaignId: TEST_CAMPAIGN_ID, folderId: TEST_FOLDER_ID });
...
describe('links', () => {
    ...
    test('get', async () => {
        const res = await api.getLink({ campaignId: TEST_CAMPAIGN_ID, linkId: TEST_LINK_ID });
...
describe('images', () => {
    ...
    test('get', async () => {
        const res = await api.getImage({ campaignId: TEST_CAMPAIGN_ID, imageId: TEST_IMAGE_ID });
